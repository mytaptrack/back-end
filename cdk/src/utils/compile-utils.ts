    ...
    Object.keys(options).sort().forEach((key: string) => {
        ...
        if (type === 'boolean') {
            ...
            if (Array.isArray(value)) {
                compilerOptionsString += option + ' ' + value.map(v => v.startsWith('./')? path.resolve(path.join('..', v)) : v).join(',') + ' ';
            }
        } else {
...
function extractTsConfig(tsconfigPath: string, previousCompilerOptions?: Record<string, any>): Record<string, any> | undefined {
    const configPathStr = path.resolve(tsconfigPath);
    logger.info('tsconfig path', configPathStr);

    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const { extends: extendedConfig, compilerOptions } = require(configPathStr);
    const updatedCompilerOptions = {
        ...compilerOptions,
        ...(previousCompilerOptions ?? {}),
    };
    if (extendedConfig) {
        const configPath = path.resolve(tsconfigPath.replace(/[^\/]+$/, ''), extendedConfig);
        logger.info('tsconfig path', configPath);
