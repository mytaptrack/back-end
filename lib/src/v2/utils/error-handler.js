...
exports.ErrorHandlerFactory = exports.ErrorAggregator = exports.DatabaseErrorHandler = exports.ErrorSeverity = void 0;
...
class DatabaseErrorHandler {
    ...
    constructor(logger) {
        this.logger = logger || database_logger_1.LoggerFactory.getLogger('DatabaseErrorHandler');
    }
    handleError(error, context) {
        // Translate the error using the appropriate translator
        const translatedError = database_errors_1.ErrorTranslatorFactory.translateError(error, context.provider);
...
class ErrorAggregator {
    ...
    constructor(maxErrors = 1000) {
        ...
        this.logger = database_logger_1.LoggerFactory.getLogger('ErrorAggregator');
    }
    addError(report) {
...
class ErrorHandlerFactory {
    ...
    static getHandler(provider) {
        if (!this.handlers.has(provider)) {
            const logger = database_logger_1.LoggerFactory.getLogger(`ErrorHandler:${provider}`);
            this.handlers.set(provider, new DatabaseErrorHandler(logger));
        }
        return this.handlers.get(provider);
    }
    static createAggregator(maxErrors) {
